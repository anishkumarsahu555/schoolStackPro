...
def _teacher_context(request):
    ...
    session_id = current.get('Id') or (teacher.sessionID_id if teacher else None)
    school_id = current.get('SchoolID') or (teacher.schoolID_id if teacher else None)
    return teacher, session_id, school_id


@login_required
@check_groups('Teaching')
def get_teacher_leave_type_list_api(request):
    teacher, session_id, _ = _teacher_context(request)
...
class TeacherLeaveApplicationListJson(BaseDatatableView):
    order_columns = [
        'leaveTypeID__name',
        'startDate',
        'endDate',
    ...
    def get_initial_queryset(self):
        teacher, session_id, _ = _teacher_context(self.request)
        if not teacher or not session_id:
...
@login_required
@check_groups('Teaching')
def get_teacher_leave_detail_api(request):
    teacher, session_id, _ = _teacher_context(request)
    if not teacher or not session_id:
...
@login_required
@check_groups('Teaching')
def teacher_apply_leave_api(request):
    if request.method != 'POST':
        return ErrorResponse('Method not allowed.', status_code=405).to_json_response()
    teacher, session_id, school_id = _teacher_context(request)
    if not teacher or not session_id:
    ...
    try:
        ...
        add_leave_log(
            ...
            school_id=school_id,
            session_id=session_id,
            actor_label=teacher.name or request.user.username,
        )
        ...
        logger.error(f'Error in teacher_apply_leave_api: {exc}')
...
@login_required
@check_groups('Teaching')
def teacher_update_leave_api(request):
    if request.method != 'POST':
        return ErrorResponse('Method not allowed.', status_code=405).to_json_response()
    teacher, session_id, school_id = _teacher_context(request)
    if not teacher or not session_id:
    ...
    try:
        ...
        logger.error(f'Error in teacher_update_leave_api: {exc}')
...
@login_required
@check_groups('Teaching')
def teacher_cancel_leave_api(request):
    if request.method != 'POST':
        return ErrorResponse('Method not allowed.', status_code=405).to_json_response()
    teacher, session_id, school_id = _teacher_context(request)
    if not teacher or not session_id:
    ...
    try:
        ...
        logger.error(f'Error in teacher_cancel_leave_api: {exc}')
